...
# Function to compute market capitalizations

def get_latest_ciruculating_supply(symbol_list):
...
def get_yesterday_prices_from_db(symbol_list):
...
def compute_market_caps_weights(symbol_list):
    # Step 1: Get the latest circulating supply from the database
    circulating_supply_dict = get_latest_ciruculating_supply(symbol_list)

    # Step 2: Attempt to fetch yesterday's prices from the database
    redstone_prices_db = get_yesterday_prices_from_db(symbol_list)
    ...
    if missing_symbols:
        print('WARNING: Missing symbols:', missing_symbols)
...
# Example usage:
symbol_list = ['BTC', 'ETH', 'UNI', 'LINK']  # Example list of symbols
symbol_list = ['BTC', 'ETH']



market_caps_df = compute_market_caps_weights(symbol_list)
